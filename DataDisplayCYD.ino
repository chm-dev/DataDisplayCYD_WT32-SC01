// Optimize for size to fit within ESP32 flash limits
#pragma GCC optimize("Os")

// WT32-SC01 version – based on DataDisplayCYD v1.4.1
...
// posix from api / right time over all world
...
// Using LovyanGFX for WT32-SC01 (supports ST7789 display and FT6336 touch)
#include <LovyanGFX.hpp>
#include "lgfx_wt32sc01.hpp"  // WT32-SC01 display configuration

// TFT_eSPI.h replaced with LovyanGFX for better WT32-SC01 support
#include <SPI.h>
#include "time.h"
// XPT2046_Touchscreen.h removed - using LovyanGFX built-in touch for WT32-SC01
...
LGFX tft;  // LovyanGFX display object for WT32-SC01
Preferences prefs;
...
// Touch uses I2C (FT6336), not SPI like CYD
#define LCD_BL_PIN 23  // Backlight pin for WT32-SC01
...
// Touch handled by LovyanGFX built-in touch (FT6336 for WT32-SC01)
// XPT2046_Touchscreen ts(T_CS, T_IRQ); - REMOVED
...
// Touch calibration - WT32-SC01 uses capacitive touch (no calibration needed like resistive)
// Touch coordinates are now handled directly by LovyanGFX
// WT32-SC01: 320x480 native, with rotation 1 (landscape) = 480x320
const int TOUCH_X_MIN = 0;
const int TOUCH_X_MAX = 479;
const int TOUCH_Y_MIN = 0;
const int TOUCH_Y_MAX = 319;
const int SCREEN_WIDTH = 480;   // WT32-SC01 width in landscape mode
const int SCREEN_HEIGHT = 320;  // WT32-SC01 height in landscape mode
    ...
    if (doc.is<JsonArray>()) {
      ...
      if (arr.size() > 0) {
        ...
        if (first["name"].is<const char*>() && first["lat"].is<const char*>() && first["lon"].is<const char*>()) {
          ...
          // Nominatim může vrátit název v lokální abecede (arabsky, čínsky...)
          // Pokud obsahuje non-ASCII znaky, použijeme jméno zadané uživatelem
          bool isAscii = true;
...
void syncRegion() {
  ...
  if (httpCode == 200) {
    ...
    if (!error && doc["status"] == "success") {
      ...
      // Detekce zeme podle casove zony
      ...
      // 3. APLIKOVAT ZMENY (Ulozi, nastavi cas - interne resetuje lat/lon na 0.0)
      applyLocation();

      // 4. PREPSAT NULY skutecnymi souradnicemi z IP geolokace
      //    applyLocation() zamerně resetuje lat/lon, proto je musime dosadit az po nem
      if (detectedLat != 0.0 || detectedLon != 0.0) {
        lat = detectedLat;
        lon = detectedLon;
        prefs.begin("sys", false);
...
String ianaToPostfixTZ(String iana) {
  ...
  if (iana.indexOf("Casablanca") >= 0 || iana.indexOf("El_Aaiun") >= 0) {
    return "WET0WEST,M3.5.0,M10.5.0/3";  // Maroko ma DST
  }
  ...
  if (iana.indexOf("Khartoum") >= 0) {
    return "CAT-3";  // Sudan UTC+3, bez DST
  }
...
// ===== TFT LCD INITIALIZATION =====
  tft.init();
  tft.setRotation(1);  // Landscape mode for WT32-SC01 (320x480)
  
  // WT32-SC01 doesn't have the same inversion quirk as CYD
  // If colors look inverted, change rgb_order in lgfx_wt32sc01.hpp
  delay(50);
  ...
  Serial.println("[SETUP] TFT initialized (WT32-SC01 with ST7789)");

  // ===== TOUCHSCREEN INITIALIZATION =====
  // WT32-SC01 uses FT6336 I2C capacitive touch - handled by LovyanGFX
  // No separate SPI initialization needed
  // Touch is initialized automatically with tft.init()
  
  Serial.println("[SETUP] Touchscreen initialized (FT6336 via LovyanGFX)");
...
void loop() {
  ...
  // 2. TOUCH HANDLING (LovyanGFX built-in touch for WT32-SC01)
